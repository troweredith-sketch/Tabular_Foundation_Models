...
作为 Big Plus 扩展，本项目还在 Adult 上研究了 TabICL 的支持集选择。实验把三个预算受限的支持集策略与 full-context 参考结果进行比较。冻结版本的 balanced prototype retrieval 策略没有超过 random subset 或 balanced random subset，但预算受限支持集显著降低了运行时间。因此，Big Plus 结果更适合被解释为一个有价值的负结果 ablation：支持集压缩在实践上有用，但当前基于类别中心的 prototype retrieval 规则不是一种能提升预测表现的方法。
...
Adult 是一个二分类收入预测数据集，包含 48,842 个样本、14 个原始特征，其中 6 个是数值特征，8 个是类别特征。它混合了数值列和类别列，并包含缺失值。在主线结果中，Adult 整体上更偏向 boosted-tree baselines，尽管 TabICL 接近最强树模型。

Bank Marketing 是一个类别不平衡更明显的二分类数据集。它可以检验 balanced accuracy 和 macro-F1 是否会给出不同于 accuracy 的结论。在主线结果中，foundation models 在这个数据集上更有竞争力，并且 TabICL 有最强的不平衡类别相关指标。
...
**图 1. Train size 与 accuracy 的关系。** 图中展示 Adult 和 Bank Marketing 上 seeds 42 到 46 的 mean accuracy。更大的训练集通常会提高 accuracy；Adult 上树模型仍然最强，而 Bank Marketing 上 TabICL 变得更有竞争力。

![图 2. Train size 与 balanced accuracy 的关系。](../results/figures/phase5_scalability_balanced_accuracy.png)

**图 2. Train size 与 balanced accuracy 的关系。** 图中展示 seeds 42 到 46 的 mean balanced accuracy。这个指标突出少数类表现，尤其是在 Bank Marketing 上，TabICL 从更大的 train sizes 中获得的收益比单看 accuracy 更明显。

![图 3. Train size 与 macro-F1 的关系。](../results/figures/phase5_scalability_macro_f1.png)

**图 3. Train size 与 macro-F1 的关系。** 图中展示 seeds 42 到 46 的 mean macro-F1。Macro-F1 进一步确认 dataset-dependent pattern：Adult 仍然更有利于 boosted trees，而 Bank Marketing 上 TabICL 拥有最强的 full-train imbalance-aware result。
...
Table 3 给出对应的 Phase 6 summary values。Metrics 是三个 seeds 的均值，runtime 是相同 seeds 上 total runtime 的中位数。

**表 3. Phase 6 Adult 支持集选择结果。**

| 策略 | Budget | Accuracy | Balanced Accuracy | Macro-F1 | Median Runtime (s) |
| --- | ---: | ---: | ---: | ---: | ---: |
...
Runtime 结果仍然有价值。Full Context 的 median total runtime 为 45.8005 秒，而预算受限策略大约在 2.5 到 4.8 秒范围内。因此，support-set compression 可以让 TabICL 快得多。然而，class-center prototype rule 不足以在这个设置中提升预测指标。
...
Big Plus 支持集选择实验给出了一个有价值的负结果。预算受限支持集显著降低了 TabICL runtime，但冻结版 balanced prototype retrieval strategy 没有在预测表现上超过 random subset 或 balanced random subset。最后的经验是保守的：TabICL 是 Bank Marketing 和实际扩展工作的一个有希望的 foundation-model baseline，但支持集检索方法必须先超过强 random 和 balanced random baselines，才能被称为真正的改进。
