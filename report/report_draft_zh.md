...
本项目没有使用 TALENT、TabArena 或其他外部表格 benchmark suite。所有报告结果都来自所选 OpenML 数据集上的本地实验，因此 Adult 和 Bank Marketing 应被理解为一个轻量 benchmark subset，而不是 suite-level benchmark coverage。
...
提交版 artifacts 在 WSL Ubuntu 中生成，使用 Python 3.12.3、AMD Ryzen 7 8845H CPU 和 NVIDIA GeForce RTX 4060 Laptop GPU。关键包版本快照为 NumPy 2.4.4、pandas 3.0.2、scikit-learn 1.8.0、LightGBM 4.6.0、XGBoost 3.2.0、PyTorch 2.11.0、TabPFN 7.1.1 和 TabICL 2.1.0。

计时口径是脚本层面的 practical timing，围绕每个模型的 fit 和 predict calls。对 LightGBM 和 XGBoost 来说，预处理在 scikit-learn pipeline 内部，因此包含在 fit/predict timing 中。对 TabPFN v2 来说，外部 median-imputation 和 ordinal-encoding preparation 发生在被计时的 model fit call 之前，计时列只覆盖模型 fit 和 predict。对 TabICL 来说，model fit 和 predict calls 包含其内部 tabular transformation 工作。模型对象构造、package import time、CSV loading、OpenML cache reads 和报告渲染不计入模型 runtime columns。

TabICL checkpoint 会在主循环前显式准备好，避免 checkpoint download 或首次加载设置污染每次运行的计时。除此之外，本项目没有执行单独的 same-device warm-up、memory benchmark 或 CPU-only/GPU-only 归一化计时流程。因此，runtime medians 应被解释为该环境下的实际本地运行成本，而不是硬件归一化速度排名。
...
Table 1 和 scalability analysis 来自不同实验输出文件。Table 1 使用 Phase 4 主线输出，而 Table 2 和 Figure 4 使用 Phase 5 scalability 输出。预测指标在共享 split protocol 下可以直接比较，但 runtime medians 应理解为独立 rerun 下的本地实际计时，不同 rerun 之间可能波动，尤其是使用 GPU 的 foundation-model runs。
...
本项目只使用两个主线数据集 Adult 和 Bank Marketing，因此结果不能泛化到所有表格分类任务。项目也不声称覆盖 TALENT 或 TabArena suite。

Missingness 鲁棒性检查也刻意保持较小规模：只覆盖 Adult、一个 train size、一个 seed，以及三个 cell-level injected missingness rates。它可以作为 injected feature missingness 下的 sanity check，但不是跨 missingness mechanisms、categorical-cardinality shifts 或更多 benchmark suites 的系统研究。
...
计划中的 Phase 7 次验证没有启动，留作 future work。这样可以避免在最终阶段加入验证不足的新实验，导致 Phase 6 已冻结的解释口径变得不清楚。
